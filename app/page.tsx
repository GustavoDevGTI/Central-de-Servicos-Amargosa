...
const officialCategoryLabels = new Set((page.segments.find((segment) => segment.type === "categories")?.items || []).filter((item) => item.type === "category").map((item) => item.label));
const audienceLabels = new Map((page.segments.find((segment) => segment.type === "audiences")?.items || []).filter((item) => item.type === "audience").map((item) => [item.id, item.label || item.id]));
...
const serviceAudienceLabel = (service: Service) => (service.audienceIds?.length ? service.audienceIds : [service.audienceId]).map((id) => audienceLabels.get(id) || id).join(" · ");
...
function Brand({ segment }: { segment: Segment }) {
  ...
  return <span className="brand home-brand">{logo?.src ? <img {...itemSizeProps(logo)} className="brand-image" src={logo.src} alt={logo.alt || "Bandeira de Amargosa"} /> : <span {...itemSizeProps(logo)} className="mark">AM</span>}<span><strong {...itemSizeProps(title)}>{title?.value || "Município de Amargosa"}</strong><small {...itemSizeProps(subtitle)}>{subtitle?.value || "Central de Serviços"}</small></span></span>;
}
