...
type Segment = { id: string; name: string; type: string; enabled: boolean; mergeWithPrevious?: boolean; size?: Size; style: { background?: string; color?: string; accent?: string; width?: string; spacing?: string; radius?: string; variant?: string; headingFont?: string; bodyFont?: string; fontSize?: string; backgroundImage?: string; backgroundImages?: string[] }; items: Item[] };
...
const itemSizeProps = (entry?: Item) => { const x = Math.round(Number(entry?.position?.x) || 0); const y = Math.round(Number(entry?.position?.y) || 0); return { style: { ...sizeStyle(entry), position: x || y ? "relative" as const : undefined, zIndex: x || y ? 2 : undefined, transform: x || y ? `translate(${x}px,${y}px)` : undefined }, "data-user-sized-item": entry?.size ? "true" : undefined, "data-user-positioned-item": entry?.position ? "true" : undefined }; };
