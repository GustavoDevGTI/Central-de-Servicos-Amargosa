...
export default function Home() {
  ...
  function renderSegment(segment: Segment): ReactNode {
    ...
    if (segment.type === "hero") { const search = items(segment, "search")[0]; const featured = segments.find((entry) => entry.type === "featured"); const shortcuts = items(featured, "serviceRef").slice(0, 4).map((ref) => services.find((service) => service.id === ref.serviceId)).filter(Boolean) as Service[]; return <section key={segment.id} id="conteudo" className={classes(segment, "hero")} style={segmentStyle(segment)}><span className="eyebrow">{text(segment, "eyebrow")}</span><h1>{text(segment, "title")}</h1>{text(segment, "description") && <p>{text(segment, "description")}</p>}<label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={search?.placeholder} aria-label="Buscar serviços" /><a href="#todos-os-servicos">{search?.buttonText || "Buscar"}</a></label><div className="popular"><span>Mais buscados:</span>{shortcuts.map((service) => <button key={service.id} type="button" onClick={() => { setQuery(service.title); setAudience("todos"); setCategory("todos"); scrollResults(); }}>{service.title}</button>)}</div>{text(segment, "notice") && <small>{text(segment, "notice")}</small>}</section>; }
    if (segment.type === "audiences") return <section key={segment.id} id="publicos" className={classes(segment, "audience-panel")} style={segmentStyle(segment)}><SectionHeading segment={segment} /><div className="audiences">{items(segment, "audience").map((item) => <button key={item.id} type="button" onClick={() => { setAudience(item.id); setCategory("todos"); scrollResults(); }}><strong>{item.label}</strong><small>{item.description}</small><b>Ver serviços →</b></button>)}</div></section>;
