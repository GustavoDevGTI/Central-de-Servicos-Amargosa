...
type Segment = { id: string; name: string; type: string; enabled: boolean; mergeWithPrevious?: boolean; size?: Size; style: { background?: string; color?: string; accent?: string; width?: string; spacing?: string; radius?: string; variant?: string; headingFont?: string; bodyFont?: string; backgroundImage?: string }; items: Item[] };
...
export default function Home() {
  ...
  function segmentStyle(segment: Segment) { return { "--segment-bg": segment.style.background, "--segment-color": segment.style.color, "--segment-accent": segment.style.accent, "--segment-heading-font": fontStacks[segment.style.headingFont || "lora"], "--segment-body-font": fontStacks[segment.style.bodyFont || "source"], backgroundImage: segment.style.backgroundImage ? `url(${JSON.stringify(segment.style.backgroundImage).slice(1, -1)})` : undefined, ...sizeStyle(segment, true) } as CSSProperties; }
