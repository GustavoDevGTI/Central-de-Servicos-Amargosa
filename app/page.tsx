...
export default function Home() {
  ...
  function renderSegment(segment: Segment): ReactNode {
    ...
    if (segment.type === "header") return <header key={segment.id} className={classes(segment, "header")} style={segmentStyle(segment)}><a href="#conteudo" aria-label="Página inicial da Central de Serviços"><Brand segment={segment} /></a><nav aria-label="Navegação principal">{items(segment, "link").map((item) => <a key={item.id} href={item.url}>{item.text}</a>)}</nav><div className="header-actions"><a className="accessibility-entry" href="/menu">Acessibilidade</a><a className="menu" href="#categorias">☰ Menu de serviços</a></div></header>;
