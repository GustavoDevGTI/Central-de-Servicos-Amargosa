...
type DirectorySort = "nameAsc" | "nameDesc" | "newest";
...
export function ServiceDirectory({ mode, value, initialQuery = "" }: { mode: Mode; value: string; initialQuery?: string }) {
  ...
  const selectionOptions = mode === "audience"
    ? [{ id: "todos", label: "Todos os serviços", href: "/servicos" }, ...audiences.filter((entry) => entry.id !== audience?.id).map((entry) => ({ id: entry.id, label: entry.label, href: `/publicos/${entry.id}` }))]
    : mode === "category"
      ? [{ id: "todos", label: "Todos os serviços", href: "/servicos" }, ...categories.filter((entry) => entry.id !== category?.id).map((entry) => ({ id: entry.id, label: entry.label, href: `/categorias/${slugify(entry.label)}` }))]
      : [];
  const selectionAction = mode === "audience" ? "Alterar público" : "Alterar categoria";
  const reset = () => { setQuery(""); setCategoryFilter(category?.label || "todos"); setAudienceFilter(audience?.id || "todos"); setDepartmentFilter("todos"); };
  return <main {...rootProps()}><a className="skip" href="#lista-servicos">Ir para os serviços</a><PortalHeader pageEntry={directoryPage}/><section className={internalClasses(introSegment, "context-heading")} style={internalStyle(introSegment)}><nav aria-label="Caminho de navegação"><Link href="/">Início</Link><span>›</span><strong>{selectionTitle}</strong></nav><h1>{internalText(introSegment, "title", "Central de serviços")}</h1></section><section className={internalClasses(searchSegment, "context-search")} style={internalStyle(searchSegment)} aria-labelledby="context-search-title"><header className={`context-selection${selectionOpen ? " is-open" : ""}`}>{mode !== "all" ? <button type="button" className="context-selection-summary" aria-expanded={selectionOpen} aria-controls="context-selection-options" onClick={() => setSelectionOpen((open) => !open)}><span className="context-selection-current"><strong id="context-search-title">{selectionTitle}</strong><small>{scoped.length} serviço{scoped.length === 1 ? "" : "s"}</small></span><span className="context-selection-action">{selectionAction}<b aria-hidden="true">{selectionOpen ? "−" : "+"}</b></span></button> : <div className="context-selection-summary context-selection-static"><span className="context-selection-current"><strong id="context-search-title">{selectionTitle}</strong><small>{scoped.length} serviço{scoped.length === 1 ? "" : "s"}</small></span></div>}{selectionOpen && selectionOptions.length > 0 && <nav id="context-selection-options" className="context-selection-options" aria-label={mode === "audience" ? "Outros públicos" : "Outras categorias"}>{selectionOptions.map((option) => <Link key={option.id} href={option.href} onClick={() => setSelectionOpen(false)}><span>{option.label}</span><b aria-hidden="true">→</b></Link>)}</nav>}</header><div className="context-filter-grid"><label className="context-query">Buscar por serviço<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={searchItem?.placeholder || "Digite para buscar"}/></label><label>Categoria<select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="todos">Todas</option>{categories.map((entry) => <option key={entry.id} value={entry.label}>{entry.label}</option>)}</select></label><label>Público<select value={audienceFilter} onChange={(event) => setAudienceFilter(event.target.value)}><option value="todos">Todos</option>{audiences.map((entry) => <option key={entry.id} value={entry.id}>{entry.label}</option>)}</select></label><label>Órgão responsável<select value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)}><option value="todos">Todos</option>{departments.map((entry) => <option key={entry} value={entry}>{entry}</option>)}</select></label><button type="button" onClick={reset}>{searchItem?.buttonText || "Limpar"}</button></div></section><section id="lista-servicos" className={internalClasses(catalogSegment, "context-results")} style={internalStyle(catalogSegment)}><header><div className="context-results-tools"><p>Exibindo <strong>{scoped.length}</strong> resultado{scoped.length === 1 ? "" : "s"}</p><div className="context-sort" aria-label="Ordenar serviços"><button type="button" className={sortMode === "nameAsc" || sortMode === "nameDesc" ? "active" : ""} aria-pressed={sortMode === "nameAsc" || sortMode === "nameDesc"} onClick={() => setSortMode((current) => current === "nameAsc" ? "nameDesc" : "nameAsc")}>Nome <b aria-hidden="true">{sortMode === "nameDesc" ? "↓" : "↑"}</b></button><button type="button" className={sortMode === "newest" ? "active" : ""} aria-pressed={sortMode === "newest"} onClick={() => setSortMode("newest")}>Novos</button></div></div><b role="status" aria-live="polite">{selectionTitle}</b></header><div className="context-service-grid">{scoped.map((service) => <Link className="context-service-card" key={service.id} href={serviceHref(service)} {...serviceLinkProps(service)}><i aria-hidden="true">+</i><span><small>{service.category}</small><strong>{service.title}</strong><em>{service.department}</em></span><b>{internalText(catalogSegment, "action", "Acessar →")}</b></Link>)}</div>{scoped.length === 0 && <p className="context-empty">{internalText(catalogSegment, "empty", "Nenhum serviço corresponde aos filtros escolhidos.")}</p>}</section><PortalFooter/></main>;
}
...
function RichServiceDetail({ service }: { service: Service }) {
  ...
  return <main {...rootProps()}>
    ...
    <article id="conteudo-servico" className={`${internalClasses(heroSegment, "service-detail")} service-detail-rich`} style={internalStyle(heroSegment)}>
      <header>
        <div>
          <small>{service.subject || service.category}</small>
          <h1>{service.title}</h1>
          <p>{service.summary}</p>
          <div className="service-identity"><AudienceTags service={service}/><span>{service.subject || service.category}</span></div>
        </div>
      </header>

      {service.notice && <a className="service-reference-notice" href={service.url} target="_blank" rel="noreferrer"><span>{service.notice}</span><small>Clique neste destaque para acessar o E-SIC oficial ↗</small></a>}
