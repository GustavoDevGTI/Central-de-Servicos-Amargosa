...
export async function GET() {
  try {
    ...
    const result = await database
      .prepare(
        `SELECT service_id AS serviceId, SUM(search_clicks) AS clicks
      ...
      .all<{ serviceId: string; clicks: number }>();
    return json(
      {
        services: result.results
          .filter((entry) => knownServiceIds.has(entry.serviceId))
          .map((entry) => ({
            serviceId: entry.serviceId,
            searches: Number(entry.clicks) || 0,
          })),
      },
      200,
      {
...
export async function POST(request: Request) {
  ...
  if (await isRateLimited(request))
    return json({ error: "Muitas requisições." }, 429, { "Retry-After": "60" });
  ...
  if (!serviceId || serviceId.length > 100 || !knownServiceIds.has(serviceId))
  ...
  try {
    ...
    await database
      .prepare(
        `INSERT INTO service_popularity_daily (day, service_id, search_clicks, updated_at)
      VALUES (?, ?, 1, ?)
      ON CONFLICT(day, service_id) DO UPDATE SET
        search_clicks = search_clicks + 1,
        updated_at = excluded.updated_at`,
      )
      .bind(day, serviceId, now.toISOString())
      .run();
