(() => {
  ...
  const categoriesFor = (audienceId, source = services) => {
    const audienceServices = source.filter((service) => serviceAudiences(service).includes(audienceId));
  ...
  const unassigned = services.filter((service) => !serviceAudiences(service).some((id) => knownAudienceIds.has(id)));
  ...
  const serviceLink = (service) => `<li><a href="../?servico=${escapeHtml(service.slug || service.id)}"><span><strong>${escapeHtml(service.title)}</strong>${service.department ? `<small>${escapeHtml(service.department)}</small>` : ""}</span><b>Ver serviço →</b></a></li>`;
